...
/// 获取到数据完成的回调
@property(nonatomic, copy)void (^completeBlock)(NSArray<KYContactObject *> * _Nonnull);
...
@implementation KYContactManager
...
-(instancetype)initContactDidChange:(void (^)(NSArray<KYContactObject *> * _Nonnull))contactDidChange
{
    if (self = [self init])
    {
...
-(NSArray<id<CNKeyDescriptor>> *)descriptors
{
    if (_descriptors == nil)
    {
        return [NSString KYContactAllKeys];
    }
 ...
 *  获取通讯录中的联系人
 */
- (void)__obtainContacts:(void (^)(NSArray<KYContactObject *> * _Nonnull)) completeBlock
{
    ...
    //进行缓存
    [[KYContactCatcheManager sharedInstace]startRequestIndentifiers:^(NSArray<NSString *> * _Nonnull identifiers) {
...
- (void)__contactDidChange:(NSNotification *)notication
{
    ...
    if (self.contactDidChange != nil )
    {
        ...
        {
            //重新加载缓存
            [[KYContactCatcheManager sharedInstace]reloadContactIdentifiers:^(NSArray<NSString *> * _Nonnull identifiers) {
...
/**
 将所有的identifiers转成KYObject对象

 @param identifiers 存放identifiers的数组

 @return 初始化完毕的KYContactObject数组
 */
- (NSArray <KYContactObject *> *)__contactHandleWithIdentifiers:(NSArray <NSString *> *)identifiers
{
    NSMutableArray <KYContactObject *> * contacts = [NSMutableArray arrayWithCapacity:0];
    ...
    for (NSString * identifier in identifiers)
    {
        ...
        //进行对象的处理
        KYContactObject * contactObject = [KYContactObjectManager contantObject:contact];
...
@implementation KYContactManager (KYAddContact)

-(void)addContact:(KYContactObject *)contact
{
    NSLog(@"KYContactManager add contact");
    
    //获得对象
    CNMutableContact * cnContact = [KYContactObjectManager cnContact:contact];
    ...
    if([self.contactStore executeSaveRequest:saveRequest error:&error])
    {
        ...
        else NSLog(@"KYContactManager add success");
    };
}
